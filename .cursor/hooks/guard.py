...
This is Cursor agent tooling, not application runtime code. It intentionally
does not integrate OpenTelemetry or remote logging. Behavior is covered by
`.cursor/hooks/tests/` (`npm run cursor:hooks:test`).
"""
...
def clean_evidence(text: str, limit: int = 140) -> str:
    ...
    text = re.sub(r"\bgh[pousr]_[A-Za-z0-9_]{30,}\b", "gh_<redacted>", text)
    text = re.sub(r"\bsk-(?:proj-)?[A-Za-z0-9_-]{20,}\b", "sk-<redacted>", text)
    text = re.sub(r"\bxox[baprs]-[A-Za-z0-9-]{20,}\b", "xox-<redacted>", text)
    return text[:limit]


def decode_stdin_bytes(raw_bytes: bytes) -> str:
    """Decode hook stdin across Windows/Cursor encoding variants."""
    if not raw_bytes:
        return ""
    ...
    for encoding in ("utf-8-sig", "utf-8", "utf-16", "utf-16-le", "utf-16-be", "cp1252"):
        try:
            ...
            continue
        if "\x00" in decoded and encoding.startswith("utf-8"):
            # Likely UTF-16 misread as UTF-8; try the next encoding.
            continue
        return decoded
...
def extract_json_object(text: str) -> str | None:
    """Extract the first JSON object when Cursor prefixes framing junk (e.g. `n++{...}`)."""
    start = text.find("{")
    end = text.rfind("}")
...
def parse_event_text(text: str) -> dict[str, Any]:
    ...
    return {
        "_invalid_json": True,
        "_raw_sha256": hashlib.sha256(text.encode("utf-8", errors="replace")).hexdigest(),
        "_raw_prefix": clean_evidence(stripped[:48]),
    }
...
def read_event() -> dict[str, Any]:
    ...
    if not raw_bytes.strip():
        return {}
    return parse_event_text(decode_stdin_bytes(raw_bytes))


def resolve_shell_command(event: dict[str, Any]) -> str:
    """Resolve the shell command from current and nested Cursor hook schemas."""
    for key in ("command", "shell_command", "cmd"):
        value = event.get(key)
        if isinstance(value, str) and value.strip():
            return value

    for nest_key in ("tool_input", "input", "arguments", "params"):
...
def secret_findings(text: str, file_path: str | None = None) -> list[Finding]:
    ...
    for rule, pattern in SECRET_PATTERNS:
        ...
        if match:
            out.append(Finding("critical", f"secret.{rule}", "Likely hardcoded credential or private key detected.", file_path, clean_evidence(match.group(0))))
    return out
...
def before_shell(event: dict[str, Any]) -> None:
    if event.get("_invalid_json") or event.get("_invalid_shape"):
        finding = Finding(
            ...
            "Malformed before-shell hook input; failing closed because the command cannot be evaluated safely.",
            evidence=str(event.get("_raw_sha256") or None),
        )
        record(event, [finding])
        emit({"permission": "deny", "user_message": finding.message, "agent_message": finding.message})
        return

    command = resolve_shell_command(event)
    if not command.strip():
        finding = Finding("critical", "shell.missing-command", "Missing shell command in before-shell hook input; failing closed.")
        record(event, [finding])
        emit({"permission": "deny", "user_message": finding.message, "agent_message": finding.message})
        return
...
# Local Cursor/Codex tooling is not an application runtime boundary.
TOOLING_PATH_MARKERS = (
    ...
    ".cursor/rules/",
    ".agents/skills/",
    "scripts/validate_cursor_pack.py",
    "scripts/sync_agent_skills.py",
    "scripts/generate-audit-coverage.mjs",
)


def is_tooling_path(path: str) -> bool:
    normalized = path.lower().replace("\\", "/")
    return any(marker in normalized for marker in TOOLING_PATH_MARKERS)
...
def after_edit(event: dict[str, Any]) -> None:
    ...
    is_new_module = (
        bool(text.strip())
        and suffix in PRODUCTION_EXTENSIONS
        and not is_test
        and not is_tooling_path(path)
    )
    if is_new_module:
